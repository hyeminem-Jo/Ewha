...
## 프로젝트 의의
오늘의 집(리뉴얼-프렌독) 프로젝트를 통해 그리드 시스템과 실제로 실무와 근접한 형태의 코드 관리 방법을 배웠는데, 이를 이화여대라는 사이트를 클론하면서 재정리 하는 시간을 가졌다. 이는 더 복잡한 레이아웃 구조를 가진 사이트를 선택하여 작업하고자하였고, 대학교 사이트가 그 조건에 맞았습니다. 하드코딩하는 연습을 위해 순수 자바스크립트로 작성하려 노력했으며 본 프로젝트는 이제껏 배워온 지식과 내용 전부를 쏟은 프로젝트라 할 수 있습니다.
